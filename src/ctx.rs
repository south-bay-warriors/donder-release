...
"#);
    ...
    #[test]
    fn ctx_calver_per_package_rejects_no_micro() {
        let dir = tempfile::tempdir().unwrap();
        let config = write_config(dir.path(), r#"
bump_files:
  - { target: cargo, path: "pkg-a/Cargo.toml", package: true, versioning: calver, calver_format: YYYY.MM.WW }
"#);
        let result = Ctx::new(config, "".to_string(), true, vec![]);
        assert_eq!(result.unwrap_err().to_string(), "calver_format must contain a MICRO segment");
    }

    #[test]
    fn ctx_calver_per_package_rejects_invalid_segment() {
        let dir = tempfile::tempdir().unwrap();
        let config = write_config(dir.path(), r#"
bump_files:
  - { target: cargo, path: "pkg-a/Cargo.toml", package: true, versioning: calver, calver_format: YYYY.QUARTER.MICRO }
"#);
        let result = Ctx::new(config, "".to_string(), true, vec![]);
        assert_eq!(
            result.unwrap_err().to_string(),
            "invalid calver_format segment 'QUARTER', valid segments: YYYY, YY, 0Y, MM, 0M, WW, 0W, MICRO",
        );
    }

    #[test]
    fn ctx_calver_per_package_inherits_global_format() {
        let dir = tempfile::tempdir().unwrap();
        let config = write_config(dir.path(), r#"
calver_format: YYYY.0M.MICRO
bump_files:
  - { target: cargo, path: "pkg-a/Cargo.toml", package: true, versioning: calver }
"#);
        let ctx = Ctx::new(config, "".to_string(), true, vec![]).unwrap();
        assert_eq!(ctx.packages[0].calver_format, "YYYY.0M.MICRO");
    }

    #[test]
    fn ctx_calver_per_package_requires_a_format() {
        let dir = tempfile::tempdir().unwrap();
        let config = write_config(dir.path(), r#"
bump_files:
  - { target: cargo, path: "pkg-a/Cargo.toml", package: true, versioning: calver }
"#);
        let result = Ctx::new(config, "".to_string(), true, vec![]);
        assert_eq!(result.unwrap_err().to_string(), "calver_format must be set when versioning is 'calver'");
    }

    #[test]
    fn ctx_semver_package_ignores_calver_format() {
        let dir = tempfile::tempdir().unwrap();
        let config = write_config(dir.path(), r#"
bump_files:
  - { target: cargo, path: "pkg-a/Cargo.toml", package: true, calver_format: MICRO.YYYY.MM }
"#);
        assert!(Ctx::new(config, "".to_string(), true, vec![]).is_ok());
    }
